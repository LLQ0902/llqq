...
<form action="orderUpdate" method="POST">
	...
	<table border="1">
		<tr>
			...
			<td>
				<!-- 
				1、将所有门店以选项的形式显示在下拉选框中				(√)
				2、而且还要将当前订单所属门店的选项设置为默认被选中
				让订单对象中记录的门店id( order.doorId ), 和 每一个门店
				对象中保存的id值( door.id )进行比较, 如果相等, 则将当前
				门店所在的option选项设置为默认被选中
				 -->
				<select id="doorId" name="doorId">
					<c:forEach items="${ dList }"  var="door">
						<option 
							${ order.doorId == door.id ? "selected='selected'" : "" }
